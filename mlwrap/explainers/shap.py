...
class SHAPExplainerBase(ExplainerBase, metaclass=abc.ABCMeta):
    ...
    def __subclasshook__(cls, subclass):
        return (hasattr(subclass, '_get_explainer') and
                callable(subclass._get_explainer) or
                NotImplemented)

    @abc.abstractmethod
    def _get_explainer(self, data_details: DataDetails) -> Union[shap.GradientExplainer,
                                                                 shap.explainers.Tree, shap.explainers.Linear]:
    ...
    def explain(self, data_details: DataDetails) -> List[Type[ExplanationResult]]:
        ...
        return [ExplanationResult(feature_importances=get_feature_importances(
            data_details=data_details, importances=x)) for x in importances]
...
class LinearSHAP(SHAPExplainerBase):
    def _get_explainer(self, data_details: DataDetails) -> shap.explainers.Linear:
        masker: shap.maskers.Independent = shap.maskers.Independent(
            data_details.background_data,
            self._config.explanation_background_samples)
