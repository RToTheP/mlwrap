...
class TestCleaning(unittest.TestCase):
    ...
    def test_clean_features_and_labels_success(self):
        ...
        config = MLConfig(model_feature_id="Colour", features=features)
    ...
    def test_clean_features_model_feature_removed(self):
        # arrange
        data = pd.DataFrame(
            {"Colour": ["Red", "Blue", "Red"], "Temperature": ["Hot", "Cold", "Cold"]}
        )
        features = [
            Feature(id="Colour", feature_type=FeatureType.Categorical),
            Feature(id="Temperature", feature_type=FeatureType.Categorical),
    ...
    def test_clean_features_string_feature_removed(self):
        ...
        # act
        status, _, cleaning_report = clean_features(data, config)
        ...
        self.assertTrue(
            any(
                x.feature == "Weather"
                and x.cleaning_type == CleaningType.feature_non_predictive
                for x in cleaning_report.cleaning_records
            )
        )
        self.assertTrue(
            any(
                x.label == "Snow" and x.feature == "Weather"
                for x in cleaning_report.cleaning_records
            )
        )
    ...
    def test_clean_features_label_removed(self):
        ...
        self.assertFalse(
            any(
                x.cleaning_type == CleaningType.feature_non_predictive
                for x in cleaning_report.cleaning_records
            )
        )
        self.assertEqual(
    ...
    def test_clean_rows_remove_no_valid_double_rows(self):
        # arrange
        data = pd.DataFrame({"Height": [np.nan], "Temperature": [99]})
        features = [
            Feature(
                id="Height",
        ...
        config = MLConfig(model_feature_id="Height", features=features)
    ...
    def test_clean_rows_allow_nan_double_rows(self):
        # arrange
        data = pd.DataFrame({"Height": [np.nan, 10], "Temperature": [99, 60]})
        features = [
            Feature(
                id="Height",
                feature_type=FeatureType.Continuous,
        ...
        self.assertEqual(
            ...
            len(
                [
                    ...
                    for x in cleaning_report.cleaning_records
                    if x.feature == "Height"
                    and x.cleaning_type == CleaningType.row_feature_out_of_range
                ]
            ),
        )
    ...
    def test_clean_rows_remove_nan_string_rows(self):
        # arrange
        data = pd.DataFrame({"Colour": [np.nan], "Temperature": ["Hot"]})
        features = [
            Feature(
                id="Colour",
    ...
    def test_clean_rows_remove_some_bad_rows(self):
        ...
        self.assertEqual(
            ...
            len(
                [
                    x.cleaning_type == CleaningType.row_feature_out_of_range
                    for x in cleaning_report.cleaning_records
                ]
            ),
        )
    ...
    def test_clean_rows_default_values_numbers(self):
        # arrange
        data = pd.DataFrame({"Number": [np.nan, 2, 3, 4, 5, 6]})
        features = [
            Feature(id="Number", feature_type=FeatureType.Continuous, default_value="1")
        ]
        config = MLConfig(model_feature_id="Number", features=features)
    ...
    def test_clean_rows_min_max_values(self):
        # arrange
        data = pd.DataFrame({"Number": [-10, 2, 3, 4, 5, 10000]})
        features = [
            Feature(
                id="Number",
                feature_type=FeatureType.Continuous,
    ...
    def test_clean_rows_remove_out_of_range_double(self):
        # arrange
        data = pd.DataFrame({"Number": [1, 2, -1, 4, 5, 6]})
        features = [
            Feature(
                id="Number",
                feature_type=FeatureType.Continuous,
    ...
    def test_regroup_labels_by_count_keep_one(self):
        ...
        data = pd.DataFrame({"Colour": colours})
        features = [
            Feature(
                id="Colour",
                feature_type=FeatureType.Categorical,
        ...
        # act
        status, data, cleaning_report = regroup_labels(data, config)
        ...
        self.assertEqual(
            ...
            len(
                [
                    x.feature == "Colour"
                    and x.cleaning_type == CleaningType.label_regrouped
                    for x in cleaning_report.cleaning_records
                ]
            ),
        )
        self.assertTrue(all(x in data.Colour.unique() for x in ["Red", "OTHER"]))
    ...
    def test_regroup_labels_by_count_keep_all(self):
        ...
        data = pd.DataFrame({"Colour": colours})
        features = [
            Feature(id="Colour", feature_type=FeatureType.Categorical, keep_n_labels=4)
        ]
        config = MLConfig(model_feature_id="Colour", features=features)
        ...
        self.assertEqual(
            ...
            len(
                [
                    x.cleaning_type == CleaningType.label_regrouped
                    for x in cleaning_report.cleaning_records
                ]
            ),
        )
        self.assertTrue(
            all(x in data.Colour.unique() for x in ["Red", "Yellow", "Blue", "Green"])
        )
    ...
    def test_regroup_labels_by_count_flat_tail(self):
        ...
        data = pd.DataFrame({"Colour": colours})
        features = [
            Feature(id="Colour", feature_type=FeatureType.Categorical, keep_n_labels=2)
        ]
        config = MLConfig(model_feature_id="Colour", features=features)
